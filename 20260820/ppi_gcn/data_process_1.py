...
def split_edges_bfs(edges, adj_set, y, train_r=0.6, val_r=0.2, seed=42):
    """
    BFS 划分：从随机种子节点出发做 BFS，
    前 train_r 比例的边归训练集，中间 val_r 归验证集，剩余归测试集。
    保证测试集是图中一个"局部密集子图"。
    """
...
def split_edges_dfs(edges, adj_set, y, train_r=0.6, val_r=0.2, seed=42):
    """
    DFS 划分：深度优先遍历图，模拟"稀疏分布未知蛋白"场景。
    """
    rng = random.Random(seed)
...
def process(seq_path: str, ppi_path: str, out_dir: str, split_mode: str = "bfs"):
    ...
    split_fn = SPLIT_FN[split_mode]
    train_idx, val_idx, test_idx = split_fn(
        [tuple(e) for e in all_edges],
        None, all_labels,
    ...
    print(f"   预测边:  {data.edge_pairs.shape}")
